...
\begin{description}
    ...
    \item[\texttt{lgp}] “linegap punctuations”，“行间标点”特性。适用于所有标点位置正常的字体（分别以三组思源字体为基准）。简中和繁中将逗号、顿号、冒号、分号以及句号悬挂到行间，而日文字体则不会悬挂冒号及分号。\textsf{ja\_JP}，\textsf{zh\_CN}与\textsf{zh\_TW}皆有。
\end{description}
